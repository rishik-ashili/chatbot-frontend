...
//     try {
//       const prompt = `Please verify the following statement using available information. If it is inaccurate, provide a concise, corrected version. If the statement is largely correct, respond with "CORRECT".\n\nStatement: "${textToVerify}"`;
...
//         config: {
//           tools: [groundingTool],
//           systemInstruction: "You are a helpful fact-checking assistant. Your goal is to verify statements and provide corrections if they are inaccurate."
//         }
//       });

//       const groundedResponse = response.text.trim();

//       if (groundedResponse.toUpperCase() === 'CORRECT' || groundedResponse.includes(textToVerify.substring(0, 50))) {
//         return null;
//       }
...
//       const context = history.map(msg => `${msg.sender === 'user' ? 'User' : 'AI'}: ${msg.text}`).join('\n\n');
